...
def prepare(rType="MAIN"):
    ...
    try:
        ...
        # Create User
        printc("Creating user thundercipher")
        os.system("adduser --system --shell /bin/false --group --disabled-login thundercipher > /dev/null")
    if not os.path.exists("/home/thundercipher"): os.mkdir("/home/thundercipher")
    return True
...
def install(rType="MAIN"):
    ...
    try: rURL = rDownloadURL % rInstall[rType]
        ...
        return False
    os.system('wget -q -O "/tmp/thundercipher.tar.gz" "%s"' % rURL)
    if os.path.exists("/tmp/thundercipher.tar.gz"):
        printc("Installing Software")
        os.system('tar -zxvf "/tmp/thundercipher.tar.gz" -C "/home/thunderciper/" > /dev/null')
        try: os.remove("/tmp/thunderciper.tar.gz")
        except: pass
        return True
...
def mysql(rUsername, rPassword):
    ...
    if os.path.exists("/etc/mysql/my.cnf"):
        if open("/etc/mysql/my.cnf", "r").read(14) == "# THUNDER CIPER": rCreate = False
    if rCreate:
    ...
    for i in range(5):
        ...
        try:
            if rDrop:
                os.system('mysql -u root%s -e "DROP DATABASE IF EXISTS tc_panel_db; CREATE DATABASE IF NOT EXISTS tc_panel_db;" > /dev/null' % rExtra)
                os.system("mysql -u root%s tc_panel_db < /home/thunderciper/iptv_xtream_codes/database.sql > /dev/null" % rExtra)
                os.system('mysql -u root%s -e "USE tc_panel_db; UPDATE settings SET live_streaming_pass = \'%s\', unique_id = \'%s\', crypt_load_balancing = \'%s\';" > /dev/null' % (rExtra, generate(20), generate(10), generate(20)))
                os.system('mysql -u root%s -e "USE tc_panel_db; REPLACE INTO streaming_servers (id, server_name, domain_name, server_ip, vpn_ip, ssh_password, ssh_port, diff_time_main, http_broadcast_port, total_clients, system_os, network_interface, latency, status, enable_geoip, geoip_countries, last_check_ago, can_delete, server_hardware, total_services, persistent_connections, rtmp_port, geoip_type, isp_names, isp_type, enable_isp, boost_fpm, http_ports_add, network_guaranteed_speed, https_broadcast_port, https_ports_add, whitelist_ips, watchdog_data, timeshift_only) VALUES (1, \'Main Server\', \'\', \'%s\', \'\', NULL, NULL, 0, 25461, 1000, \'%s\', \'eth0\', 0, 1, 0, \'\', 0, 0, \'{}\', 3, 0, 25462, \'low_priority\', \'\', \'low_priority\', 0, 1, \'\', 1000, 25463, \'\', \'[\"127.0.0.1\",\"\"]\', \'{}\', 0);" > /dev/null' % (rExtra, getIP(), getVersion()))
                os.system('mysql -u root%s -e "USE tc_panel_db; REPLACE INTO reg_users (id, username, password, email, member_group_id, verified, status) VALUES (1, \'admin\', \'\$6\$rounds=20000\$thunderciper\$XThC5OwfuS0YwS4ahiifzF14vkGbGsFF1w7ETL4sRRC5sOrAWCjWvQJDromZUQoQuwbAXAFdX3h3Cp3vqulpS0\', \'admin@website.com\', 1, 1, 1);" > /dev/null'  % rExtra)
            ...
            try: os.remove("/home/thunderciper/iptv_thunder/database.sql")
            except: pass
            return True
        except: printc("Invalid password! Try again", col.FAIL)
    return False

def encrypt(rHost="127.0.0.1", rUsername="user_iptvpro", rPassword="", rDatabase="tc_panel_db", rServerID=1, rPort=7999):
    printc("Encrypting...")
    try: os.remove("/home/thunderciper/iptv_thunder/config")
    except: pass
    rf = open('/home/thunderciper/iptv_thunder/config', 'wb')
...
def configure():
    ...
    if not "/home/thunderciper/iptv_thunder/" in open("/etc/fstab").read():
        rFile = open("/etc/fstab", "a")
        rFile.write("tmpfs /home/thunderciper/iptv_thunder/streams tmpfs defaults,noatime,nosuid,nodev,noexec,mode=1777,size=90% 0 0\ntmpfs /home/thunderciper/iptv_thunder/tmp tmpfs defaults,noatime,nosuid,nodev,noexec,mode=1777,size=2G 0 0")
        rFile.close()
    if not "thunderciper" in open("/etc/sudoers").read():
        os.system('echo "thunderciper ALL = (root) NOPASSWD: /sbin/iptables" >> /etc/sudoers')
    if not os.path.exists("/etc/init.d/thunderciper"):
        rFile = open("/etc/init.d/thunderciper", "w")
        rFile.write("#! /bin/bash\n/home/thunderciper/iptv_thunder/start_services.sh")
        rFile.close()
        os.system("chmod +x /etc/init.d/thunderciper > /dev/null")
    try: os.remove("/usr/bin/ffmpeg")
    except: pass
    os.system("ln -s /home/thunderciper/iptv_thunder/bin/ffmpeg /usr/bin/")
    os.system("chown thunderciper:thunderciper -R /home/thunderciper > /dev/null")
    os.system("chmod -R 0777 /home/thunderciper > /dev/null")
    os.system("chmod +x /home/thunderciper/iptv_thunder/start_services.sh > /dev/null")
...
if __name__ == "__main__":
    ...
    if rType.upper() in ["MAIN", "LB"]:
        ...
        if os.path.exists("/home/thunderciper/iptv_panel/wwwdir/api.php"):
